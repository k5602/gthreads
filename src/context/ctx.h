...
extern "C"
{
    ...
    {
        void *regs[8];
        void *fxsave_area; /* 16-byte aligned, 512 bytes for SSE/x87 state */
    } gth_ctx_t;

    /*
     * Save current context into `from`, restore from `to`.
     * `to` must have been initialized by gth_ctx_make or a previous gth_ctx_swap.
     * Returns 0 when resumed; does not return when switching to a fresh context.
     */
     ...
     */
    void gth_ctx_make(gth_ctx_t *ctx, void *stack_top, void (*entry)(int), int slot_index);
